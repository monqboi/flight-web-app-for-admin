...
const legendItemsPopularAirlines = ref([]);
const flightsScheduleAnalysisData = ref(null);
...
onMounted(async () => {
  try {
    ...
    const bookingsRes = await axios.get('/api/dashboard/bookings');
    bookings.value = bookingsRes.data;

    const barRes = await axios.get('/api/dashboard/chart/bar');
    chartData.value = barRes.data;

    const pieRes = await axios.get('/api/dashboard/chart/pie');
    pieChartData.value = pieRes.data;

    const legendRes = await axios.get('/api/dashboard/airlines');
    legendItemsPopularAirlines.value = legendRes.data;

    const lineRes = await axios.get('/api/dashboard/chart/line');
    flightsScheduleAnalysisData.value = lineRes.data;
...
const updatedChartData = computed(() => ({
  ...
  datasets: [{
    ...
    backgroundColor: [...Array(6).fill(getComputedStyle(document.documentElement).getPropertyValue("--c-light-blue")),
...
const updatedFlightsScheduleAnalysisData = computed(() => ({
  ...flightsScheduleAnalysisData.value,
  datasets: flightsScheduleAnalysisData.value?.datasets?.map((ds, idx) => ({
    ...ds,
