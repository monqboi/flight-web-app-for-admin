...
const aircraftData = computed(() => props.aircrafts); 

const itemsPerPage = 6;
const currentPage = ref(1);

const totalPages = computed(() =>
  Math.ceil(aircraftData.value.length / itemsPerPage)
);
...
const paginatedAircrafts = computed(() => {
  ...
  return aircraftData.value.slice(start, end);
});

watch(paginatedAircrafts, (val) => {
  console.log(" paginatedAircrafts updated and emitted:", val);
});

onMounted(() => {
  console.log("Aircrafts received as props:", props.aircrafts);
  emit("update:paginatedData", paginatedAircrafts.value);
});
